    ...
    params[:group_user].each do |user|
      ...
      if(user.has_key?(:phone) && user[:phone] != '')
        ...
        if @group_invite.save
          ...
          @message = "Hey it’s #{@group_invite.invitee.first_name}! Join my Caresplit group, #{@group_invite.group.name}. We can swap childcare with each other using this- it's free! Here’s the link: app.caresplit.com/users/sign_up"
